...
def create_tables1(dbname):
    try:
        ...
        cur.execute("""CREATE TABLE export_ine_csv(
            ...
            total varchar
        """)
        cur.execute("""CREATE TABLE export_ine_education_modificado_csv(
            sexo varchar,
            ccaa varchar,
...
def fill_tables1(dbname):
    ...
    try:
        ...
        with open(f_ine, 'r') as f:
            ...
            next(reader) # Skip the header row.
            for row in reader:
                cur.execute("INSERT INTO df_atur_barcelona_exploitation VALUES (%s, %s, %s, %s, %s)", row)

        with open(f_ine_educ, 'r') as f:
            reader = csv.reader(f)
            next(reader) # Skip the header row.
            for row in reader:
                cur.execute("INSERT INTO df_atur_zone_exploitation VALUES (%s, %s, %s, %s, %s)", row)
