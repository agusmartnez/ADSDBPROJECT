...
# ANÁLISIS:
# table 1 --> mirar si el paro depende del sector de trabajo, del genero, en que CCAA aumenta más. Hacer modelo para predecir el año que viene.
# table 2 --> comparar paro entre barcelona (cataluña: barcelona) y todo Cataluña. Mirar en qué trimestre dentro de un año hay más paro en España. 
# Mirar teniendo en cuenta el covid, cuando aumenta el paro. Mirar como hubiera sido el año 2020 si no hubiera habido covid, haciendo predicciones con modelos de los años anteriores.
# table 3 --> crear nueva variable (rate) a partir del 2015 con (atur registrat/demanda no aturats) y mirar que barrios tiene mas paro.
...
"""Around 31.8% MAPE implies the model is about 68.2% accurate in predicting the  observations corresponding to the years 2020-2021. 
...
df_atur_barcelona = df_atur_barcelona[:24506] #2015
...
df_rate = df_atur.merge(df_no_atur, on=['Year', 'quarter', 'name_district', 'name_neighborhood', 'gender'], how='inner')

df_rate2021 = df_rate[df_rate['Year']==2021]
df_rate2021 = df_rate2021.groupby(by=['name_neighborhood']).agg({'total_x':'sum', 'total_y': 'sum'})
df_rate2021 = df_rate2021.reset_index()
df_rate2021['Rate'] = df_rate2021['total_x']/df_rate2021['total_y']*100

df_rate2021 = df_rate2021.replace({'el Poble Sec':'el Poble-sec'})
...
f = open('./gdrive/MyDrive/ADSDB Project/landing/persistent/4. feature generation zone/barris.geojson')
barris_pol = json.load(f)
f.close()
max_count = df_rate2021['Rate'].max()*0.3
min_count = df_rate2021['Rate'].min()
fig = px.choropleth_mapbox(df_rate2021,
                           geojson=barris_pol, 
